...
$active_page = "manage_bookings.php";
...
if (isset($_POST['update_booking'])) {
    ...
    $sql = "UPDATE bookings SET payment_status = '$status' WHERE booking_id = '$booking_id'";

    if (mysqli_query($conn, $sql)) {
        $_SESSION['success_message'] = "Booking payment status updated successfully!";
    } else {
...
// Get all bookings
$sql = "SELECT b.*, u.username, u.email, p.name as package_name, p.price as package_price
        FROM bookings b 
...
<head>
    ...
    <style>
        ...
        tr[style*="display: none"] .delete-btn,
        tr[style*="display: none"] .edit-btn {
            display: inline-flex !important;
            visibility: visible !important;
            opacity: 1 !important;
        }
...
<body class="bg-gray-50">
    <div class="flex">
        ...
        <div class="flex-1 lg:ml-64">
            ...
            <main class="p-6">
                ...
                <!-- Bookings Table Only - No Stats Cards -->
                                        ...
                                        <?php while ($booking = mysqli_fetch_assoc($result)): ?>
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    ...
                                                    <div class="text-gray-400 text-xs"><?php echo htmlspecialchars($booking['email']); ?></div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                    <div class="action-buttons">
                                                        <button onclick="openEditModal(<?php echo htmlspecialchars(json_encode($booking), ENT_QUOTES, 'UTF-8'); ?>)" 
                                                                class="edit-btn text-blue-600 hover:text-blue-900 bg-blue-100 hover:bg-blue-200 px-3 py-1 rounded text-xs">
                                                            Edit Status
                                                        </button>
                                                        <form method="POST" action="" class="action-form" onsubmit="return confirm('Are you sure you want to delete booking #<?php echo $booking['booking_id']; ?>?');">
    ...
    <script>
        function openEditModal(bookingData) {
            ...
            document.getElementById('booking_id').value = bookingData.booking_id;
            document.getElementById('status').value = bookingData.payment_status;
        }
